; S05E03 CX-50 Keyboard Controllers - Example 1
;
...
; overview 
;
; handling keyboards - in this example we read both port-a and port-b keyboards on every retrace.
; you could instead check the keyboards after several retraces, or switch which keyboard you check
; on even/odd number of retraces.
;
; config keyboard - you need to set the data pins on SWACNT (switch A controller) to be output. in
; this example we set only the pins for the keyboard we're going to check. this is not nessesary
; and takes extra time. (see example 2) it is built this way to fully demonstrate how the register
; works. you could instead set all the pins to output once. d0-d3 are for port-a, d4-d7 for port-b.
;
; powering rails - SWCHA tells the pia what rows of the keyboard to supply power. you should only
; power one row at a time or you won't be clear which key on the same column was pressed. you
; set the bit to 0 for the row you want to power, and the rest to 1. this example only powers the
; pins on the keyboard we are currently reading. this is not nessesary and takes extra time. you
; you could instead power both keyboards at the same time and check for button  presses on both for
; each row. (see example 2) SWCHA uses d0-d3 for port-a, d4-d7 for port-b. after writing to SWCHA
; the hardware needs 400 microseconds (about 477 machine cycles) to power the row.
;
; reading input - input for each column for the row that was powered with SWCHA, can be read with
; INPT0, INPT1, and INPT4 for port-a and INTP2, INPT5, and INPT3 for port-b. You determine what
; key was press based on which row you're powering, and what input is positive.
;
; example 2 includes some optimization.
;
...
keyRowMask_ptr   ds 2                   ; pointer to row masks for current keyboard
pf1a             ds 1                   ; left pf1
pf2a             ds 1                   ; left pf2
pf0b             ds 1                   ; right pf0
pf1b             ds 1                   ; right pf1
...
; start of overscan
                 ...
                 lda #1                 ; start with keyboard 1 (port b)
                 sta keyboard

keyboards        ldy keyboard           ; get which keyboard we're going to activate
                 lda pins_mask,y        ; get the bit mask telling SWACNT which pins are
                 sta SWACNT             ; input/output. Technically, you can set then all to

                 lda keyRowMaskLSB,y    ; build the pointer to one of the sets of KeyRowMasks
                 sta keyRowMask_ptr     ; based on which keyboard we're reading
                 lda keyRowMaskMSB,y
                 sta keyRowMask_ptr+1

                 lda #3                 ; start with bottom row
                 sta row
rows             ldy row                ; load the current row
                 lda (keyRowMask_ptr),y ; load the row

rowMaskDone      sta SWCHA              ; need to wait 400 microseconds before you can read inputs
                 ldx #6                 ; 477 mc = 6.3 sl
wait             sta WSYNC
                 ; ----------------------
                 dex
                 ...
                 ; check each of the columns to see if there's a key press
                 ldx rowOffset,y        ; offsets for chars index based on which row we're reading
...
p0_col2          lda INPT1              ; if keypress char index = rowOffset + 1
                 bmi p0_col3
                 inx                    ; add 1 to rowOffset
                 jmp pressed            ; key press detected
p0_col3          lda INPT4              ; if keypress char index = rowOffset + 2
                 bmi nopress
                 inx                    ; add 2 to rowOffset
                 inx                    ; key press detected
                 jmp pressed
...
pressed          lda #1
                 ...
                 stx char               ; set chat char index was calculated
                 jmp doneKeys           ; done with reading keyboards

nopress          dec row                ; no key press detected, move to next row
                 bpl rows

                 dec keyboard           ; to key press detects on any rows, move to next keyboard
                 bpl keyboards
...
pins_mask        .byte %11110000        ; p0 keyboard
                 .byte %00001111        ; p1 keyboard

keys_row_mask1   .byte %11101111        ; power first row
                 .byte %11011111        ; power second tow
                 .byte %10111111        ; power third row
                 .byte %01111111        ; power forth row

keys_row_mask2   .byte %11111110        ; power first row
                 .byte %11111101        ; power second tow
                 .byte %11111011        ; power third row
                 .byte %11110111        ; power forth row

keyRowMaskLSB    .byte <keys_row_mask1  ; create pointers to rom addresses so we can access the
                 .byte <keys_row_mask2  ; each set of masks as a list

keyRowMaskMSB    .byte >keys_row_mask1
                 .byte >keys_row_mask2

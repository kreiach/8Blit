        processor 6502          ; S03E02 Ex. 2 4 tones, 2 channels, audio subroutines
                                ;
        include	 "vcs.h"	; Demonstrates audio generation by setting the audio registers to create 
        include  "macro.h"      ; a sounds when the ball bounces, on reset, on arena selection forward and
                                ; backwards. Turns volume off after a specified number of frames per sound.
                                ;
...
;--------------------------------------	 
        ...
        bne noball              ; 3  27 - if it's not the same then continue with disabled ball graphic
        ldx #2                  ; 2  29 - ball y pos and playfield counter are same so enable ball graphic
...
int_inv:        
        ...
        ldx #1                  ; channel 1
        ldy #2                  ; sound parameter index from sndbank_*
        jsr snd_play            ; call the subroutine to load the audio registers
...
int_advance:
        ...
        ldx #1                  ; channel 1
        ldy #3                  ; sound parameter index from sndbank_*
        jsr snd_play            ; call the subroutine to load the audio registers
        jmp change_arena
